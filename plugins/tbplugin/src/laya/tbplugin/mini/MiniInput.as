package laya.tbplugin.mini
{
	...
	public class MiniInput {
		...
		private static function _createInputElement():void {
			...
			//
			//TBPluginAdapter.window.my.onWindowResize && TBPluginAdapter.window.my.onWindowResize(function(res:*):void {
				//TBPluginAdapter.window.dispatchEvent && TBPluginAdapter.window.dispatchEvent("resize");
			//});
			...
			//运行环境判断
			var model:String= TBPluginAdapter.systemInfo.model;
			var system:String = TBPluginAdapter.systemInfo.system;
		...
		public static function wxinputFocus(e:*):void {
			...
			//TBPluginAdapter.window.my.offKeyboardConfirm();
			//TBPluginAdapter.window.my.offKeyboardInput();
			TBPluginAdapter.window.my.showKeyboard({defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: 'done', success: function(res:*):void {
			}, fail: function(res:*):void {
			}});
			
			TBPluginAdapter.window.my.onKeyboardConfirm(function(res:*):void {
				var str:String = res ? res.value : "";
				_inputTarget.text = str;
				_inputTarget.event(Event.INPUT);
				MiniInput.inputEnter(true);
			})
			TBPluginAdapter.window.my.onKeyboardInput(function(res:*):void {
				var str:String = res ? res.value : "";
		...
		public static function hideKeyboard():void {
			...
			//TBPluginAdapter.window.my.offKeyboardConfirm();
			//TBPluginAdapter.window.my.offKeyboardInput();
			TBPluginAdapter.window.my.hideKeyboard({success: function(res:*):void {
				console.log('隐藏键盘')
			}, fail: function(res:*):void {
