package laya.tbgame.mini
{
	...
	public class MiniSound extends EventDispatcher {
		...
		public function load(url:String):void {
			...
			this.url = url;
			this.readyUrl = url;
			if(TBMiniGameAdapter.autoCacheFile&&MiniFileMgr.getFileInfo(url))
			{
				...
				}else
				{
					...
					}else
					{
						...
						|| url.indexOf(TBMiniGameAdapter.window.my.env.USER_DATA_PATH) != -1) {
							...
							MiniFileMgr.downOtherFiles(url, Handler.create(this, onDownLoadCallBack, [url]), url, TBMiniGameAdapter.autoCacheFile);
						}
					}
				}
			}
		...
		private function onDownLoadCallBack(sourceUrl:String,errorCode:int,tempFilePath:String = ""):void
		{
			if (!errorCode && this._sound)
			{
				...
				{
					if((MiniFileMgr.isLocalNativeFile(sourceUrl) && !MiniFileMgr.isSubNativeFile(sourceUrl))||
					(
