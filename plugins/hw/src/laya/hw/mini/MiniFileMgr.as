package laya.hw.mini {
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = HWMiniAdapter.window.hbs.getFileSystemManager();
		/**@private 下载文件接口**/
		private static var wxdown:* = HWMiniAdapter.window.hbs.downloadFile;
		/**@private 文件缓存列表**/
		...
		{
			for(var i:int = 0,sz:int = HWMiniAdapter.nativefiles.length;i<sz;i++)
			{
		...
		{
			var memSize:int = HWMiniAdapter.minClearSize;
		...
		private static function writeFilesList(fileurlkey:String,filesListStr:String,isAdd:Boolean):void
		{
			...
			{
				HWMiniAdapter.window.hbs.postMessage({url:fileurlkey,data:filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
			}
		}
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = HWMiniAdapter.window.hbs.env.USER_DATA_PATH + value;
		}
	}
}
