	...
	public class TTMiniAdapter {
		...
		public static function init(isPosMsg:Boolean = false,isSon:Boolean = false):void {
			...
			if(!isZiYu)
			{
				...
				MiniFileMgr.existDir(MiniFileMgr.fileNativeDir, Handler.create(TTMiniAdapter, onMkdirCallBack));
			}
			systemInfo = window.tt.getSystemInfoSync();
			...
			window.CanvasRenderingContext2D.prototype = window.tt.createCanvas().getContext('2d').__proto__;
...
//			MiniLocation.__init__();
			openCtx = window.tt.getOpenDataContext();
			onReciveData();
		...
		private static function onReciveData():void
		{
			...
			{
				window.tt.onMessage(function(message:Object):void{
		...
		public static function createElement(type:String):* {
			if (type == "canvas") {
				...
				if (idx == 1) {
					...
					_source = window.tt.createCanvas();
				}
				idx++;
				return _source;
		...
		private static function postInfoToContext(url:String,atlaspngUrl:String,atlasJson:Object):void
		{
			...
			if(fileNativeUrl)
			{
				openCtx.postMessage({url:url,atlasdata:postData,imgNativeUrl:fileNativeUrl,imgReadyUrl:textureUrl,isLoad:"opendatacontext"});
			}else
			{
		...
		public static function sendSinglePicToOpenDataContext(url:String):void
		{
			...
			if(fileNativeUrl)
			{
				openCtx.postMessage({url:url,imgNativeUrl:fileNativeUrl,imgReadyUrl:url,isLoad:"openJsondatacontextPic"});
			}else
			{
				throw "获取图集的磁盘url路径不存在！";
			}
		...
		public static function sendJsonDataToDataContext(url:String):void
		{
			if(!TTMiniAdapter.isZiYu)
			{
				...
				if(atlasJson)
				{
					openCtx.postMessage({url:url,atlasdata:atlasJson,isLoad:"openJsondatacontext"});
				}else
				{
					throw "传递的url没有获取到对应的图集数据信息，请确保图集已经过！";
				}
