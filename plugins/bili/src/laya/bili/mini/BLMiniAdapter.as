package laya.bili.mini {
	...
	public class BLMiniAdapter {
		public static var IGNORE:RegExp = new RegExp("[-_.!~*'();/?:@&=+$,#%]|[0-9|A-Z|a-z]");
		/**@private  包装对象**/
		...
		public static function init(isPosMsg:Boolean = false,isSon:Boolean = false):void {
			...
			if(!isZiYu)
			{
				...
				MiniFileMgr.existDir(MiniFileMgr.fileNativeDir, Handler.create(BLMiniAdapter, onMkdirCallBack));
			}
			systemInfo = window.bl.getSystemInfoSync();
			...
			HttpRequest._urlEncode = function(str:String):String
			{
				var  strTemp:String = "";
				var  length:int = str.length;
				for (var i:int = 0; i < length; i++)
				{
					var word:String=str[i];
					if (IGNORE.test(word))
					{
						strTemp += word;
					}
			...
			window.CanvasRenderingContext2D.prototype = window.bl.createCanvas().getContext('2d').__proto__;
...
//			MiniLocation.__init__();
		...
		private static function onReciveData():void
		{
			...
			{
				window.bl.onMessage(function(message:Object):void{
		...
		private static function postInfoToContext(url:String,atlaspngUrl:String,atlasJson:Object):void
		{
			...
			if(fileNativeUrl)
			{
				window.bl.postMessage({url:url,atlasdata:postData,imgNativeUrl:fileNativeUrl,imgReadyUrl:textureUrl,isLoad:"opendatacontext"});
			}else
			{
		...
		public static function sendSinglePicToOpenDataContext(url:String):void
		{
			...
			if(fileNativeUrl)
			{
				window.bl.postMessage({url:url,imgNativeUrl:fileNativeUrl,imgReadyUrl:url,isLoad:"openJsondatacontextPic"});
			}else
			{
				throw "获取图集的磁盘url路径不存在！";
			}
		...
		public static function sendJsonDataToDataContext(url:String):void
		{
			if(!BLMiniAdapter.isZiYu)
			{
				...
				if(atlasJson)
				{
					window.bl.postMessage({url:url,atlasdata:atlasJson,isLoad:"openJsondatacontext"});
				}else
				{
					throw "传递的url没有获取到对应的图集数据信息，请确保图集已经过！";
				}
