package laya.yk.mini
{
	...
	public class YKMiniAdapter {
		...
		public static function init(isPosMsg:Boolean = false,isSon:Boolean = false):void {
			...
			if(!window.hasOwnProperty("my"))
				return;
			if(window.navigator.userAgent.indexOf('AlipayMiniGame') <0 ) return;
			isZiYu = isSon;
			isPosMsgYu = isPosMsg;
			EnvConfig = {};
			//设置资源存储目录
			if(!isZiYu)
			{
				...
				MiniFileMgr.existDir(MiniFileMgr.fileNativeDir, Handler.create(YKMiniAdapter, onMkdirCallBack));
			}
			...
			window.CanvasRenderingContext2D.prototype = window.my.createCanvas().getContext('2d').__proto__;
		...
		{
			//接收主域透传的数据
			if(YKMiniAdapter.isZiYu)
			{
				window.my.onMessage(function(message:Object):void{
		...
		public static function createElement(type:String):* {
			if (type == "canvas") {
				...
				if (idx == 1) {
					...
					_source = window.my.createCanvas();
					_source.style = {};
				}
				idx++;
				return _source;
		...
		private static function postInfoToContext(url:String,atlaspngUrl:String,atlasJson:Object):void
		{
			...
			if(fileNativeUrl)
			{
				window.my.postMessage({url:url,atlasdata:postData,imgNativeUrl:fileNativeUrl,imgReadyUrl:textureUrl,isLoad:"opendatacontext"});
			}else
			{
		...
		public static function sendSinglePicToOpenDataContext(url:String):void
		{
			...
			if(fileNativeUrl)
			{
				window.my.postMessage({url:url,imgNativeUrl:fileNativeUrl,imgReadyUrl:url,isLoad:"openJsondatacontextPic"});
			}else
			{
				throw "获取图集的磁盘url路径不存在！";
			}
		...
		public static function sendJsonDataToDataContext(url:String):void
		{
			if(!YKMiniAdapter.isZiYu)
			{
				...
				if(atlasJson)
				{
					window.my.postMessage({url:url,atlasdata:atlasJson,isLoad:"openJsondatacontext"});
				}else
				{
					throw "传递的url没有获取到对应的图集数据信息，请确保图集已经过！";
				}
