package laya.yk.mini
{
	...
	public class MiniInput {
		...
		private static function _createInputElement():void {
			...
			YKMiniAdapter.window.my.onWindowResize && YKMiniAdapter.window.my.onWindowResize(function(res:*):void {
				YKMiniAdapter.window.dispatchEvent && YKMiniAdapter.window.dispatchEvent("resize");
			});
			...
			//运行环境判断
			var model:String= YKMiniAdapter.systemInfo.model;
			var system:String = YKMiniAdapter.systemInfo.system;
		...
		public static function wxinputFocus(e:*):void {
			...
			YKMiniAdapter.window.my.offKeyboardConfirm();
			YKMiniAdapter.window.my.offKeyboardInput();
			YKMiniAdapter.window.my.showKeyboard({defaultValue: _inputTarget.text, maxLength: _inputTarget.maxChars, multiple: _inputTarget.multiline, confirmHold: true, confirmType: 'done', success: function(res:*):void {
			}, fail: function(res:*):void {
			}});
			
			YKMiniAdapter.window.my.onKeyboardConfirm(function(res:*):void {
				var str:String = res ? res.value : "";
				_inputTarget.text = str;
				_inputTarget.event(Event.INPUT);
				MiniInput.inputEnter(true);
			})
			YKMiniAdapter.window.my.onKeyboardInput(function(res:*):void {
				var str:String = res ? res.value : "";
		...
		public static function hideKeyboard():void {
			YKMiniAdapter.window.my.offKeyboardConfirm();
			YKMiniAdapter.window.my.offKeyboardInput();
			YKMiniAdapter.window.my.hideKeyboard({success: function(res:*):void {
				console.log('隐藏键盘')
			}, fail: function(res:*):void {
