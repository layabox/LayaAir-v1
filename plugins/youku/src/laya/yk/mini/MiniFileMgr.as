package laya.yk.mini
{
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = YKMiniAdapter.window.my.getFileSystemManager();
		/**@private 下载文件接口**/
		private static var wxdown:* = YKMiniAdapter.window.my.downloadFile;
		/**@private 文件缓存列表**/
		...
		{
			for(var i:int = 0,sz:int = YKMiniAdapter.nativefiles.length;i<sz;i++)
			{
				//优化调整  if(url.indexOf(MiniAdpter.nativefiles[i]) == 0)
				if(url.indexOf(YKMiniAdapter.nativefiles[i]) != -1)
					return true;
			}
			return false;
		}
		...
		public static function readFile(filePath:String, encoding:String = "utf8", callBack:Handler = null, readyUrl:String = "",isSaveFile:Boolean = false,fileType:String = "",isAutoClear:Boolean =true):void {
			fs.readFile({filePath: filePath, encoding: encoding, success: function(data:Object):void {
				...
				{
					if(YKMiniAdapter.autoCacheFile || isSaveFile)
					{
						callBack != null && callBack.runWith([0, data]);
		...
		public static function downOtherFiles(fileUrl:String, callBack:Handler = null, readyUrl:String = "",isSaveFile:Boolean = false,isAutoClear:Boolean = true):void {
			wxdown({url: fileUrl, success: function(data:Object):void {
				...
				if (data.statusCode === 200) {
					if((YKMiniAdapter.autoCacheFile || isSaveFile ) && readyUrl.indexOf(".php") == -1){
		...
		{
			if(YKMiniAdapter.window.navigator.userAgent.indexOf('AlipayMiniGame') <0)
			{
				...
				if (fileObj.readyUrl != readyUrl)
				{
					fs.getFileInfo({
						...
						success:function(data:Object):void
						{
							...
							{
								if(data.size > YKMiniAdapter.minClearSize)
									YKMiniAdapter.minClearSize = data.size;
		...
		{
			var memSize:int = YKMiniAdapter.minClearSize;
		...
		private static function writeFilesList(fileurlkey:String,filesListStr:String,isAdd:Boolean):void
		{
			...
			//主域向子域传递消息
			if(!YKMiniAdapter.isZiYu &&YKMiniAdapter.isPosMsgYu && YKMiniAdapter.window.my.postMessage)
			{
				YKMiniAdapter.window.my.postMessage({url:fileurlkey,data:filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
			}
		}
...
//				callBack != null && callBack.runWith([0, {data: filesListStr}]);
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = YKMiniAdapter.window.my.env.USER_DATA_PATH + value;
		}
	}
}
