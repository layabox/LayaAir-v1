package laya.yk.mini
{
	...
	public class MiniLoader  extends EventDispatcher  {
		...
		private function load(url:String, type:String = null, cache:Boolean = true, group:String = null, ignoreCache:Boolean = false):void {
			...
			var encoding:String = YKMiniAdapter.getUrlEncode(url,type);
			var urlType:String = Utils.getFileExtension(url);
			if ((_fileTypeArr.indexOf(urlType) != -1) || type == Loader.IMAGE) {
				//图片通过miniImage去加载
				YKMiniAdapter.EnvConfig.load.call(this, url, type, cache, group, ignoreCache);
			} else {
				//如果是子域,并且资源单独的图集列表文件里不存在当前路径的数据信息，这时需要针对url进行一次转义
				if(YKMiniAdapter.isZiYu && !MiniFileMgr.ziyuFileData[url])
				{
				...
				if (!MiniFileMgr.getFileInfo(url)) {
					if (MiniFileMgr.isLocalNativeFile(url)) {
						if (YKMiniAdapter.subNativeFiles && YKMiniAdapter.subNativeheads.length == 0)
						{
							for (var key:* in YKMiniAdapter.subNativeFiles)
							{
								var tempArr:Array = YKMiniAdapter.subNativeFiles[key];
								YKMiniAdapter.subNativeheads = YKMiniAdapter.subNativeheads.concat(tempArr);
								for (var aa:int = 0; aa < tempArr.length;aa++)
								{
									YKMiniAdapter.subMaps[tempArr[aa]] = key + "/" + tempArr[aa];
								}
							}
						}
						//判断当前的url是否为分包映射路径
						if(YKMiniAdapter.subNativeFiles && url.indexOf("/") != -1)
						{
							var curfileHead:String = url.split("/")[0]  +"/";//文件头
							if(curfileHead && YKMiniAdapter.subNativeheads.indexOf(curfileHead) != -1)
							{
								var newfileHead:String = YKMiniAdapter.subMaps[curfileHead];
					...
					if (tempurl.indexOf(YKMiniAdapter.window.my.env.USER_DATA_PATH) == -1 &&(url.indexOf("http://") != -1 || url.indexOf("https://") != -1)&& !YKMiniAdapter.AutoCacheDownFile) {
						//远端文件加载走xmlhttprequest
						YKMiniAdapter.EnvConfig.load.call(thisLoader, tempUrl, type, cache, group, ignoreCache);
					} else {
						...
						{
							YKMiniAdapter.EnvConfig.load.call(thisLoader, url, type, cache, group, ignoreCache);
						}
						else
						{
							url = URL.formatURL(url);
							...
							{
								MiniFileMgr.downFiles(encodeURI(url), encoding, new Handler(MiniLoader, onReadNativeCallBack, [encoding, url, type, cache, group, ignoreCache, thisLoader]), url,true);
							}
						}
					}
				} else {
		...
		private static function onReadNativeCallBack(encoding:String, url:String, type:String = null, cache:Boolean = true, group:String = null, ignoreCache:Boolean = false, thisLoader:* = null, errorCode:int = 0, data:Object = null):void {
			if (!errorCode) {
				...
				if (type == Loader.JSON || type == Loader.ATLAS) {
					tempData = YKMiniAdapter.getJson(data.data);
				...
				//主域向子域派发数据
				if(!YKMiniAdapter.isZiYu &&YKMiniAdapter.isPosMsgYu && type  != Loader.BUFFER && YKMiniAdapter.window.wx)
				{
					YKMiniAdapter.window.my.postMessage({url:url,data:tempData,isLoad:"filedata"});
				}
				thisLoader.onLoaded(tempData);
			} else if (errorCode == 1) {
				//远端文件加载走xmlhttprequest
