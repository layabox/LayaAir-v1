package laya.tb.mini
{
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = TBMiniAdapter.window.my.getFileSystemManager();
		/**@private 下载文件接口**/
		private static var wxdown:* = TBMiniAdapter.window.my.downloadFile;
		/**@private 文件缓存列表**/
		...
		{
			for(var i:int = 0,sz:int = TBMiniAdapter.nativefiles.length;i<sz;i++)
			{
		...
		{
			for(var i:Number = 0,sz:Number = TBMiniAdapter.subNativeheads.length;i<sz;i++)
			{
				//优化调整
		...
		{
			var memSize:int = TBMiniAdapter.minClearSize;
		...
		private static function writeFilesList(fileurlkey:String,filesListStr:String,isAdd:Boolean):void
		{
			...
			{
				TBMiniAdapter.window.my.postMessage({url:fileurlkey,data:filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
			}
		}
...
//				callBack != null && callBack.runWith([0, {data: filesListStr}]);
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = TBMiniAdapter.window.my.env.USER_DATA_PATH + value;
		}
	}
}
