package laya.qq.mini {
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = QQMiniAdapter.window.qq.getFileSystemManager();
		/**@private 下载文件接口**/
		private static var wxdown:* = QQMiniAdapter.window.qq.downloadFile;
		/**@private 文件缓存列表**/
		...
		{
			for(var i:int = 0,sz:int = QQMiniAdapter.nativefiles.length;i<sz;i++)
			{
				//优化调整  if(url.indexOf(QQMiniAdapter.nativefiles[i]) == 0)
		...
		{
			var memSize:int = QQMiniAdapter.minClearSize;
		...
		public static function deleteFile(tempFileName:String, readyUrl:String = "", callBack:Handler = null,encoding:String = "",fileSize:int = 0):void {
			...
			if (!fileObj) 
			{
				onSaveFile(readyUrl, "", false, encoding, callBack);
				return;
			}
		...
		private static function writeFilesList(fileurlkey:String,filesListStr:String,isAdd:Boolean):void
		{
			...
			{
				QQMiniAdapter.window.qq.postMessage({url:fileurlkey,data:filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
			}
		}
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = QQMiniAdapter.window.qq.env.USER_DATA_PATH + value;
		}
	}
}
