package laya.bd.mini {
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = __JS__('swan.getFileSystemManager()');
		/**@private 下载文件接口**/
		private static var wxdown:* = __JS__('swan.downloadFile');
		/**@private 文件缓存列表**/
		 ...
		 * @param fileType 文件类型
		 */
		public static function downFiles(fileUrl:String, encoding:String = "utf8", callBack:Handler = null, readyUrl:String = "",isSaveFile:Boolean = false,fileType:String = ""):void {
		 ...
		 * @param fileType 文件类型
		 */
		public static function readFile(filePath:String, encoding:String = "utf8", callBack:Handler = null, readyUrl:String = "",isSaveFile:Boolean = false,fileType:String = ""):void {
			fs.readFile({filePath: filePath, encoding: encoding, success: function(data:Object):void {
				if (readyUrl.indexOf("http://") != -1 || readyUrl.indexOf("https://") != -1)
				{
					...
					{
						callBack != null && callBack.runWith([0, data]);
						copyFile(filePath, readyUrl, null,encoding);
					}else
					{
						callBack != null && callBack.runWith([0, data]);
					}
			...
			wxdown({url: fileUrl, success: function(data:Object):void {
				if (data.statusCode === 200) {
					if((BMiniAdapter.autoCacheFile || isSaveFile))
					{
						...
						copyFile(data.tempFilePath, readyUrl, null);
					}
					else
						callBack != null && callBack.runWith([0, data.tempFilePath]);
				}
...
//					onSaveFile(readyUrl, tempFileName,isAdd,encoding,callBack,fileSize);//清理文件列表
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = __JS__('swan.env.USER_DATA_PATH') + value;
		}
	}
}
