package {
	...
	public class LayaTest {
		 ...
		 * 保存测试结果的文件路径，用于程序解析。
		 */
		private var jsonFilePath:String = "fps.dist.json";
		/**
		 * 本地存储 key 。
		 */
		...
		private function allTested():void {
			...
			data.push(calcRes);
			NodeFile.writeFile(jsonFilePath, JSON.stringify(data), function() {
		...
		private function analyseFile(data:Array = null):void {
			...
			if (!data) {
				var fileData:* = NodeFile.readFile(jsonFilePath);
				fileData && fileData.length && (data = JSON.parse(fileData));
			}
			trace(data);
			...
			for (i = 0; i < col; i++) {
				tempText = Laya.stage.addChild(new Text());
