...
var MiniFileMgr$11=(function(){
	...
	__class(MiniFileMgr,'laya.tb.mini.MiniFileMgr',null,'MiniFileMgr$11');
	MiniFileMgr.isLocalNativeFile=function(url){
		for(var i=0,sz=TBMiniGameAdapter.nativefiles.length;i<sz;i++){
			if(url.indexOf(TBMiniGameAdapter.nativefiles[i])!=-1)
				return true;
		}
		return false;
	}
	...
	MiniFileMgr.isSubNativeFile=function(url){
		for(var i=0,sz=TBMiniGameAdapter.subNativeheads.length;i<sz;i++){
			if(url.indexOf(TBMiniGameAdapter.subNativeheads[i])!=-1)
				return true;
		}
		return false;
	}
				...
				MiniFileMgr.fs.getFileInfo({
					...
					success:function (data){
						if((isAutoClear && (fileUseSize+chaSize+data.size)>=totalSize)){
							if(data.size > TBMiniGameAdapter.minClearSize)
								TBMiniGameAdapter.minClearSize=data.size;
	...
	MiniFileMgr.onClearCacheRes=function(){
		var memSize=TBMiniGameAdapter.minClearSize;
		var tempFileListArr=[];
	...
	MiniFileMgr.writeFilesList=function(fileurlkey,filesListStr,isAdd){
		...
		if(!TBMiniGameAdapter.isZiYu &&TBMiniGameAdapter.isPosMsgYu && TBMiniGameAdapter.window.my.postMessage){
			TBMiniGameAdapter.window.my.postMessage({url:fileurlkey,data:MiniFileMgr.filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
		}
	}
	...
	MiniFileMgr.setNativeFileDir=function(value){
		MiniFileMgr.fileNativeDir=TBMiniGameAdapter.window.my.env.USER_DATA_PATH+value;
	}
	...
	['fs',function(){return this.fs=TBMiniGameAdapter.window.my.getFileSystemManager();},'wxdown',function(){return this.wxdown=TBMiniGameAdapter.window.my.downloadFile;}
	]);
	return MiniFileMgr;
})()
...
var MiniImage$11=(function(){
	function MiniImage(){}
	__class(MiniImage,'laya.tb.mini.MiniImage',null,'MiniImage$11');
	...
	__proto._loadImage=function(url){
		...
		if (MiniFileMgr$11.isLocalNativeFile(url)|| (url.indexOf("http://")==-1 && url.indexOf("https://")==-1)){
			if (url.indexOf(TBMiniGameAdapter.window.my.env.USER_DATA_PATH)!=-1){
	...
	MiniImage.onCreateImage=function(sourceUrl,thisLoader,isLocal,tempFilePath){
		...
		if (!isLocal){
			...
			if (MiniFileMgr$11.isSubNativeFile(sourceUrl)){
				fileNativeUrl=sourceUrl;
			}else
			fileNativeUrl=TBMiniGameAdapter.baseDir+sourceUrl;
		}
...
var MiniInput$11=(function(){
	function MiniInput(){}
	__class(MiniInput,'laya.tb.mini.MiniInput',null,'MiniInput$11');
	MiniInput._createInputElement=function(){
		...
		var model=TBMiniGameAdapter.systemInfo.model;
		var system=TBMiniGameAdapter.systemInfo.system;
		...
		TBMiniGameAdapter.window.my.prompt({
			...
			success:function (res){
				if (res.ok){
					...
					laya.tb.mini.MiniInput.inputEnter();
	...
	MiniInput.hideKeyboard=function(){
		return
		TBMiniGameAdapter.window.my.hideKeyboard({success:function (res){
				console.log('隐藏键盘')
				},fail:function (res){
...
var MiniLocalStorage$10=(function(){
	...
	__class(MiniLocalStorage,'laya.tb.mini.MiniLocalStorage',null,'MiniLocalStorage$10');
	...
	MiniLocalStorage.setItem=function(key,value){
		try{
			TBMiniGameAdapter.window.my.setStorageSync({
				key:key,
				data:value
			});
		}
		catch(error){
			TBMiniGameAdapter.window.my.setStorage({
				key:key,
				data:value
			});
		}
	...
	MiniLocalStorage.getItem=function(key){
		var data=TBMiniGameAdapter.window.my.getStorageSync({"key":key});
		if (data.success)
			return data.data;
		return null;
	}
	...
	MiniLocalStorage.removeItem=function(key){
		TBMiniGameAdapter.window.my.removeStorageSync(key);
	}

	MiniLocalStorage.clear=function(){
		TBMiniGameAdapter.window.my.clearStorageSync();
	}

	MiniLocalStorage.getStorageInfoSync=function(){
		try {
			var res=TBMiniGameAdapter.window.my.getStorageInfoSync()
...
var MiniLocation$11=(function(){
	...
	__class(MiniLocation,'laya.tb.mini.MiniLocation',null,'MiniLocation$11');
	MiniLocation.__init__=function(){
		TBMiniGameAdapter.window.navigator.geolocation.getCurrentPosition=MiniLocation.getCurrentPosition;
		TBMiniGameAdapter.window.navigator.geolocation.watchPosition=MiniLocation.watchPosition;
		TBMiniGameAdapter.window.navigator.geolocation.clearWatch=MiniLocation.clearWatch;
	}
	...
	MiniLocation.getCurrentPosition=function(success,error,options){
		...
		paramO.fail=error;
		TBMiniGameAdapter.window.my.getLocation(paramO);
...
var MiniVideo$9=(function(){
	function MiniVideo(width,height){
		...
		this.videoElement=TBMiniGameAdapter.window.my.createVideo({width:width,height:height,autoplay:true});
	}

	__class(MiniVideo,'laya.tb.mini.MiniVideo',null,'MiniVideo$9');
...
//class laya.tb.mini.TBMiniGameAdapter
var TBMiniGameAdapter=(function(){
	function TBMiniGameAdapter(){}
	__class(TBMiniGameAdapter,'laya.tb.mini.TBMiniGameAdapter');
	TBMiniGameAdapter.getJson=function(data){
		return JSON.parse(data);
	}

	TBMiniGameAdapter.init=function(){
		if (TBMiniGameAdapter._inited)return;
		TBMiniGameAdapter._inited=true;
		TBMiniGameAdapter.window=/*__JS__ */window;
		var u=TBMiniGameAdapter.window.navigator.userAgent;
		if(!(u.indexOf('TB')>-1 || u.indexOf('Taobao')>-1 || u.indexOf('TM/')>-1))
			return;
		TBMiniGameAdapter.EnvConfig={};
		MiniFileMgr$11.setNativeFileDir("/layaairGame");
		MiniFileMgr$11.existDir(MiniFileMgr$11.fileNativeDir,Handler.create(TBMiniGameAdapter,TBMiniGameAdapter.onMkdirCallBack));
		TBMiniGameAdapter.systemInfo=TBMiniGameAdapter.window.my.getSystemInfoSync();
		TBMiniGameAdapter.window.focus=function (){
		};
		Laya['_getUrlPath']=function (){
		};
		Laya['getUrlPath']=function (){
		};
		TBMiniGameAdapter.window.logtime=function (str){
		};
		TBMiniGameAdapter.window.alertTimeLog=function (str){
		};
		TBMiniGameAdapter.window.resetShareInfo=function (){
		};
		TBMiniGameAdapter._preCreateElement=Browser.createElement;
		Browser["createElement"]=TBMiniGameAdapter.createElement;
		TBMiniGameAdapter.window.CanvasRenderingContext2D=function (){};
		TBMiniGameAdapter.window.CanvasRenderingContext2D.prototype=TBMiniGameAdapter._preCreateElement('canvas').getContext('2d').__proto__;
		TBMiniGameAdapter.window.document.body.appendChild=function (){
		};
		RunDriver.createShaderCondition=TBMiniGameAdapter.createShaderCondition;
		Utils['parseXMLFromString']=TBMiniGameAdapter.parseXMLFromString;
		Input['_createInputElement']=MiniInput$11['_createInputElement'];
		TBMiniGameAdapter.EnvConfig.load=Loader.prototype.load;
	...
	TBMiniGameAdapter.measureText=function(str){
		var tempObj=TBMiniGameAdapter._measureText(str);
		if(!tempObj){
			tempObj={width:16};
	...
	TBMiniGameAdapter.getUrlEncode=function(url,type){
		if(type=="arraybuffer")
			return "";
		return "utf8";
	}

	TBMiniGameAdapter.downLoadFile=function(fileUrl,fileType,callBack,encoding){
		(fileType===void 0)&& (fileType="");
		(encoding===void 0)&& (encoding="utf8");
	...
	TBMiniGameAdapter.remove=function(fileUrl,callBack){
		MiniFileMgr$11.deleteFile("",fileUrl,callBack,"",0);
	}

	TBMiniGameAdapter.removeAll=function(){
		MiniFileMgr$11.deleteAll();
	}

	TBMiniGameAdapter.hasNativeFile=function(fileUrl){
		return MiniFileMgr$11.isLocalNativeFile(fileUrl);
	}

	TBMiniGameAdapter.getFileInfo=function(fileUrl){
		return MiniFileMgr$11.getFileInfo(fileUrl);
	}

	TBMiniGameAdapter.getFileList=function(){
		return MiniFileMgr$11.filesListObj;
	}

	TBMiniGameAdapter.exitMiniProgram=function(){
		TBMiniGameAdapter.window.my.exitMiniProgram();
	}

	TBMiniGameAdapter.onMkdirCallBack=function(errorCode,data){
		if (!errorCode){
	...
	TBMiniGameAdapter.pixelRatio=function(){
		if (!TBMiniGameAdapter.EnvConfig.pixelRatioInt){
			try {
				TBMiniGameAdapter.EnvConfig.pixelRatioInt=TBMiniGameAdapter.systemInfo.pixelRatio;
				return TBMiniGameAdapter.systemInfo.pixelRatio;
			}catch (error){}
		}
		return TBMiniGameAdapter.EnvConfig.pixelRatioInt;
	}

	TBMiniGameAdapter.createElement=function(type){
		if (type=="canvas"){
			var _source;
			if (TBMiniGameAdapter.idx==1){
				_source=TBMiniGameAdapter.window.screencanvas;
				}else {
				_source=TBMiniGameAdapter._preCreateElement(type);
			}
			(!_source.style)&& (_source.style={});
			TBMiniGameAdapter.idx++;
			return _source;
			}else if (type=="textarea" || type=="input"){
			return TBMiniGameAdapter.onCreateInput(type);
			}else if (type=="div"){
			var node=TBMiniGameAdapter._preCreateElement(type);
			...
			return TBMiniGameAdapter._preCreateElement(type);
		}
	}

	TBMiniGameAdapter.onCreateInput=function(type){
		var node=TBMiniGameAdapter._preCreateElement(type);
	...
	TBMiniGameAdapter.createShaderCondition=function(conditionScript){
		var _$this=this;
		var func=function (){
	...
	TBMiniGameAdapter.EnvConfig=null;
	TBMiniGameAdapter.window=null;
	TBMiniGameAdapter._preCreateElement=null;
	TBMiniGameAdapter._inited=false;
	TBMiniGameAdapter.systemInfo=null;
	TBMiniGameAdapter.isPosMsgYu=false;
	TBMiniGameAdapter.autoCacheFile=true;
	TBMiniGameAdapter.minClearSize=(5 *1024 *1024);
	TBMiniGameAdapter.subNativeFiles={};
	TBMiniGameAdapter.subNativeheads=[];
	TBMiniGameAdapter.subMaps=[];
	TBMiniGameAdapter.AutoCacheDownFile=false;
	TBMiniGameAdapter.baseDir="";
	TBMiniGameAdapter._measureText=null;
	TBMiniGameAdapter.parseXMLFromString=function(value){
		var rst;
		var Parser;
	...
	TBMiniGameAdapter.idx=1;
	__static(TBMiniGameAdapter,
	['nativefiles',function(){return this.nativefiles=["layaNativeDir"];}
	]);
	return TBMiniGameAdapter;
})()
...
var MiniAccelerator$11=(function(_super){
	...
	__class(MiniAccelerator,'laya.tb.mini.MiniAccelerator',_super,'MiniAccelerator$11');
	...
	*/
	...
	MiniAccelerator.startListen=function(callBack){
		...
		try{
			TBMiniGameAdapter.window.my.onAccelerometerChange(laya.tb.mini.MiniAccelerator.onAccelerometerChange);
		}catch(e){}
	}
...
var MiniLoader$11=(function(_super){
	...
	__class(MiniLoader,'laya.tb.mini.MiniLoader',_super,'MiniLoader$11');
	...
	*/
	__proto.load=function(url,type,cache,group,ignoreCache){
		...
		if (MiniFileMgr$11.isLocalNativeFile(url)&& !MiniFileMgr$11.getFileInfo(url)){
			if (TBMiniGameAdapter.subNativeFiles && TBMiniGameAdapter.subNativeheads.length==0){
				for (var key in TBMiniGameAdapter.subNativeFiles){
					var tempArr=TBMiniGameAdapter.subNativeFiles[key];
			...
			if(TBMiniGameAdapter.subNativeFiles && url.indexOf("/")!=-1){
				...
				if(curfileHead && TBMiniGameAdapter.subNativeheads.indexOf(curfileHead)!=-1){
					var newfileHead=TBMiniGameAdapter.subMaps[curfileHead];
					url=url.replace(curfileHead,newfileHead);
				}
			}
		};
		var encoding=TBMiniGameAdapter.getUrlEncode(url,type);
		...
		if ((MiniLoader._fileTypeArr.indexOf(urlType)!=-1)|| type==/*laya.net.Loader.IMAGE*/"image"){
			...
			if (!MiniFileMgr$11.getFileInfo(url)){
				if (MiniFileMgr$11.isLocalNativeFile(url)){
					if (MiniFileMgr$11.isSubNativeFile(url)){
						MiniFileMgr$11.readFile(ResourceVersion.addVersionPrefix(url),encoding,new Handler(MiniLoader,MiniLoader.onReadNativeCallBack,[encoding,url,type,cache,group,ignoreCache,thisLoader]));
					}else
					MiniFileMgr$11.readFile(TBMiniGameAdapter.baseDir+ResourceVersion.addVersionPrefix(url),encoding,new Handler(MiniLoader,MiniLoader.onReadNativeCallBack,[encoding,url,type,cache,group,ignoreCache,thisLoader]));
					}else{
					var tempUrl=url;
					...
					if(fileObj){
						...
						if(type !=/*laya.net.Loader.IMAGE*/"image" && ((url.indexOf("http://")==-1 && url.indexOf("https://")==-1))){
							MiniFileMgr$11.readFile(TBMiniGameAdapter.baseDir+url,encoding,new Handler(MiniLoader,MiniLoader.onReadNativeCallBack,[encoding,url,type,cache,group,ignoreCache,thisLoader]),url);
							}else{
							MiniFileMgr$11.downFiles(encodeURI(url),encoding,new Handler(MiniLoader,MiniLoader.onReadNativeCallBack,[encoding,url,type,cache,group,ignoreCache,thisLoader]),url,TBMiniGameAdapter.AutoCacheDownFile);
						}
					}
				}
				}else {
...
var MiniSound$11=(function(_super){
	...
	__class(MiniSound,'laya.tb.mini.MiniSound',_super,'MiniSound$11');
	...
	*/
	__proto.load=function(url){
		...
		this.url=url;
		this.readyUrl=url;
		if(TBMiniGameAdapter.autoCacheFile&&MiniFileMgr$11.getFileInfo(url)){
			this.onDownLoadCallBack(url,0);
			}else{
			if(!TBMiniGameAdapter.autoCacheFile){
				this.onDownLoadCallBack(url,0);
				}else{
				if (MiniFileMgr$11.isLocalNativeFile(url)){
					...
					if ((url.indexOf("http://")==-1 && url.indexOf("https://")==-1)
						|| url.indexOf(TBMiniGameAdapter.window.my.env.USER_DATA_PATH)!=-1){
						this.onDownLoadCallBack(url,0);
						}else{
						MiniFileMgr$11.downOtherFiles(url,Handler.create(this,this.onDownLoadCallBack,[url]),url,TBMiniGameAdapter.autoCacheFile);
					}
				}
			}
		}
			...
			if(TBMiniGameAdapter.autoCacheFile){
				if(tempFilePath==""){
					if (MiniFileMgr$11.isLocalNativeFile(sourceUrl)){
						...
						if (MiniFileMgr$11.isSubNativeFile(fileNativeUrl)){
							fileNativeUrl=fileNativeUrl;
						}else
						fileNativeUrl=TBMiniGameAdapter.baseDir+fileNativeUrl;
						}else{
						...
						if(fileObj && fileObj.md5){
							...
							&& sourceUrl.indexOf(TBMiniGameAdapter.window.my.env.USER_DATA_PATH)==-1){
								fileNativeUrl=TBMiniGameAdapter.baseDir+sourceUrl;
								}else {
								fileNativeUrl=sourceUrl;
							}
				...
				if((MiniFileMgr$11.isLocalNativeFile(sourceUrl)&& !MiniFileMgr$11.isSubNativeFile(sourceUrl))||
					(
				sourceUrl.indexOf("http://")==-1
				&&sourceUrl.indexOf("https://")==-1
				&&sourceUrl.indexOf(TBMiniGameAdapter.window.my.env.USER_DATA_PATH)==-1)){
					sourceUrl=TBMiniGameAdapter.baseDir+sourceUrl;
				}
	...
	*/
	...
	MiniSound._createSound=function(){
		if(MiniSound._audioCache.length){
			...
			MiniSound._id++;
			return TBMiniGameAdapter.window.my.createInnerAudioContext();
		}
	}
...
var MiniSoundChannel$11=(function(_super){
	...
	__class(MiniSoundChannel,'laya.tb.mini.MiniSoundChannel',_super,'MiniSoundChannel$11');
