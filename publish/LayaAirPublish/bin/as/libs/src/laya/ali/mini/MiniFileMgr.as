package laya.ali.mini
{
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = ALIMiniAdapter.window.my.getFileSystemManager();
		/**@private 下载文件接口**/
		private static var wxdown:* = ALIMiniAdapter.window.my.downloadFile;
		/**@private 文件缓存列表**/
		...
		{
			for(var i:int = 0,sz:int = ALIMiniAdapter.nativefiles.length;i<sz;i++)
			{
		...
		{
			var memSize:int = ALIMiniAdapter.minClearSize;
		...
		private static function writeFilesList(fileurlkey:String,filesListStr:String,isAdd:Boolean):void
		{
			...
			{
				ALIMiniAdapter.window.my.postMessage({url:fileurlkey,data:filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
			}
		}
...
//				callBack != null && callBack.runWith([0, {data: filesListStr}]);
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = ALIMiniAdapter.window.my.env.USER_DATA_PATH + value;
		}
	}
}
