package laya.bd.mini{
	...
	public class BMiniAdapter {
		...
		public static function init(isPosMsg:Boolean = false,isSon:Boolean = false):void {
			...
			if(!isZiYu)
			{
				systemInfo = window.swan.getSystemInfoSync();
			}
		...
		private static function onReciveData():void
		{
			...
			{
				window.swan.onMessage(function(message:Object):void{
		 ...
		 * 处理预加载 
		 */		
		private static function onChuLiYuJiaZai():void
		{
			if(window.preloadResources)
			{
				var preloadResources:Array = window.preloadResources;
				for(var i:int = 0,sz:int = preloadResources.length;i<sz;i++)
				{
					var resObj:Object = preloadResources[i];
					var isExsit:Boolean = false;
					try
					{
						MiniFileMgr.fs.accessSync(resObj.filePath);
						isExsit = true;
					} 
					catch(error:Error) {};
					if(isExsit && resObj.filePath)
					{
						trace("获取预加载的资源路径 url:" + resObj.url);
						var tempFileName:String = resObj.filePath.split(MiniFileMgr.fileNativeDir+"/")[1];//获取文件名信息
						MiniFileMgr.fakeObj[resObj.url] = {md5: tempFileName, readyUrl: resObj.url,size:0,times:Browser.now(),encoding:"utf8"};//存入当前使用的缓存对象
						nativefiles.push(resObj.url);//放到本地包
					}
				}
			}
		}
		...
		public static function createElement(type:String):* {
			if (type == "canvas") {
				...
				if (idx == 1) {
					...
					_source = window.swan.createCanvas();
				}
				idx++;
				return _source;
		...
		public static function sendSinglePicToOpenDataContext(url:String):void
		{
			...
			if(fileNativeUrl)
			{
				window.swan.postMessage({url:url,imgNativeUrl:fileNativeUrl,imgReadyUrl:url,isLoad:"openJsondatacontextPic"});
			}else
			{
				throw "获取图集的磁盘url路径不存在！";
			}
		...
		public static function sendJsonDataToDataContext(url:String):void
		{
			if(!BMiniAdapter.isZiYu)
			{
				...
				if(atlasJson)
				{
					window.swan.postMessage({url:url,atlasdata:atlasJson,isLoad:"openJsondatacontext"});
				}else
				{
					throw "传递的url没有获取到对应的图集数据信息，请确保图集已经过！";
				}
