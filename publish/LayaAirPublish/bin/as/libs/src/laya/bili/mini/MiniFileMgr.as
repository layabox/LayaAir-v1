package laya.bili.mini {
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = BLMiniAdapter.window.bl.getFileSystemManager();
		/**@private 下载文件接口**/
		private static var wxdown:* = BLMiniAdapter.window.bl.downloadFile;
		/**@private 文件缓存列表**/
		...
		{
			for(var i:int = 0,sz:int = BLMiniAdapter.nativefiles.length;i<sz;i++)
			{
				//优化调整  if(url.indexOf(BLMiniAdapter.nativefiles[i]) == 0)
		...
		{
			var memSize:int = BLMiniAdapter.minClearSize;
		...
		private static function writeFilesList(fileurlkey:String,filesListStr:String,isAdd:Boolean):void
		{
			...
			{
				BLMiniAdapter.window.bl.postMessage({url:fileurlkey,data:filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
			}
		}
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = BLMiniAdapter.window.bl.env.USER_DATA_PATH + value;
		}
	}
}
