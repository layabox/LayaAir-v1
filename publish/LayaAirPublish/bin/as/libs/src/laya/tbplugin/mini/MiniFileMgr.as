package laya.tbplugin.mini
{
	...
	public class MiniFileMgr{
		/**@private 读取文件操作接口**/
		private static var fs:* = TBPluginAdapter.window.my.getFileSystemManager();
		/**@private 下载文件接口**/
		private static var wxdown:* = TBPluginAdapter.window.my.downloadFile;
		/**@private 文件缓存列表**/
		...
		{
			for(var i:int = 0,sz:int = TBPluginAdapter.nativefiles.length;i<sz;i++)
			{
		...
		{
			var memSize:int = TBPluginAdapter.minClearSize;
		...
		private static function writeFilesList(fileurlkey:String,filesListStr:String,isAdd:Boolean):void
		{
			...
			{
				TBPluginAdapter.window.my.postMessage({url:fileurlkey,data:filesListObj[fileurlkey],isLoad:"filenative",isAdd:isAdd});
			}
		}
...
//				callBack != null && callBack.runWith([0, {data: filesListStr}]);
		...
		public static function setNativeFileDir(value:String):void {
			fileNativeDir = TBPluginAdapter.window.my.env.USER_DATA_PATH + value;
		}
	}
}
