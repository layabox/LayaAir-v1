	...
	public class HWMiniAdapter {
		...
		public static function init(isPosMsg:Boolean = false,isSon:Boolean = false):void {
			...
			if(!isZiYu)
			{
				...
				MiniFileMgr.existDir(MiniFileMgr.fileNativeDir, Handler.create(HWMiniAdapter, onMkdirCallBack));
			}
			systemInfo = window.hbs.getSystemInfoSync();
		...
		private static function postInfoToContext(url:String,atlaspngUrl:String,atlasJson:Object):void
		{
			...
			if(fileNativeUrl)
			{
				window.hbs.postMessage({url:url,atlasdata:postData,imgNativeUrl:fileNativeUrl,imgReadyUrl:textureUrl,isLoad:"opendatacontext"});
			}else
			{
		...
		public static function sendSinglePicToOpenDataContext(url:String):void
		{
			...
			if(fileNativeUrl)
			{
				window.hbs.postMessage({url:url,imgNativeUrl:fileNativeUrl,imgReadyUrl:url,isLoad:"openJsondatacontextPic"});
			}else
			{
				throw "获取图集的磁盘url路径不存在！";
			}
		...
		public static function sendJsonDataToDataContext(url:String):void
		{
			if(!HWMiniAdapter.isZiYu)
			{
				...
				if(atlasJson)
				{
					window.hbs.postMessage({url:url,atlasdata:atlasJson,isLoad:"openJsondatacontext"});
				}else
				{
					throw "传递的url没有获取到对应的图集数据信息，请确保图集已经过！";
				}
